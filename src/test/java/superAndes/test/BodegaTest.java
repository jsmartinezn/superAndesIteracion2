...
//			e.printStackTrace();
    	...
    	try
		{
			...
			VOBodega bodega1 = superAndes.adicionarBodega(idSucursal, "CONGELADOS", 30.0, 20.0);
			List<Bodega> lista = superAndes.darBodegas(idSucursal);
			assertEquals ("Debe haber una bodega creada !!", 1, lista.size ());
			// Lectura de los tipos de bebida con dos tipos de bebida adicionados
			VOBodega bodega2 = superAndes.adicionarBodega(idSucursal, "CONGELADOS", 10.0, 15.0);
			lista = superAndes.darBodegas(idSucursal);
			assertEquals ("Debe haber dos tipos de bebida creados !!", 2, lista.size ());
			Bodega bod1 = lista.get(0);
			Bodega bod2 = lista.get(1);
			...
			List<Object[]> temp = superAndes.unidadesEnInv(idSucursal, producto.getCodigoDeBarras());
			assertTrue(temp.isEmpty());
			
			//Se agregar la relacion y se revisa si fue agregado correctamente
			VOBodegaProducto bodegap = superAndes.adicionarBodegaProducto(bod2.getIdSucursal(), producto.getCodigoDeBarras(), 4);
			temp = superAndes.unidadesEnInv(idSucursal, producto.getCodigoDeBarras());
			assertTrue(!temp.isEmpty());
